...
import { Member, Project, Roles, Skill, Task } from "@/gql/graphql";
...
export default function Profile(props: Props) {
  ...
  const [lastThreeProjects, setLastThreeProjects] = useState<Project[]>([])
  const [selectedProject, setSelectedProject] = useState<Project | null>(null);
  const {user , setUser} = useGlobalContext();
  useEffect(() => {
    if(props.projects?.length > 3){
      setLastThreeProjects(props.projects?.slice(props.projects?.length - 3, props.projects?.length))
    }else{
      setLastThreeProjects(props?.projects)
    }
  }
  , [props.projects])
...
<>
{
  !(user?.role == Roles.Manager || user?.role == Roles.Member) ?  
   <ErrorPage/>
:
  <>
  {modal ? (
    <CreateProject
      skills={props.skills}
      members={
        props.members && props.members.filter((member) => member.role == Roles.Manager)
      }
      setModal={setModal}
      modal={modal}
            ...
            <div className="relative flex flex-col h-full min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
              ...
              <div className="flex-auto p-4 max-h-80 overflow-x-auto">
                ...
                <ul className="flex flex-col pl-0 mb-0 rounded-lg">
                  ...
                  <li className="relative block px-4 py-2 pl-0 leading-normal bg-white border-0 border-t-0 text-sm text-inherit">
                    <strong className="text-slate-700">Employed:</strong>{" "}
          ...
          <div className="w-full max-w-full px-3 lg-max:mt-6 xl:w-4/12">
            <div className="relative flex flex-col h-full min-w-0 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
              <div className="p-4 pb-0 mb-0 bg-white border-b-0 rounded-t-2xl flex justify-between">
                <h6 className="mb-0">
                  {
                    user.role == Roles.Manager ? "Projects" : "Tasks"
                  }
                </h6>
                <select
                  id="countries"
                ...
                <ul className="flex flex-col pl-0 mb-0 rounded-lg">
                 {
                  ...
                  user.managedProjects.map((project: Project) => {
                    ...
                    return(
                      ...
                      >
                        { (projectCompletion ? projectCompletion : 0) / (tasksCount ? tasksCount : 1) * 100  > 100 ? "" : "View" }
                      </a>
                    </li>
                    )
                  }) : "No projects to view"
          ...
          <div className="flex-none w-full max-w-full px-3 mt-6">
            <div className="relative flex flex-col min-w-0 mb-6 break-words bg-white border-0 shadow-soft-xl rounded-2xl bg-clip-border">
              ...
              <div className="flex-auto p-4">
                <div className="flex flex-wrap -mx-3">
                 {
                  lastThreeProjects && lastThreeProjects.map((project , index) => {
                    return  (
                      ...
                      <div className="relative flex flex-col min-w-0 break-words bg-transparent border-0 shadow-none rounded-2xl bg-clip-border">
                        ...
                        <div className="flex-auto px-1 pt-6">
                          ...
                          <div className="flex items-center justify-between">
                            <button
                              onClick={() => {setViewProject(true) ; setSelectedProject(project)}}
                              type="button"
                 ...
                 {
                  user.role == Roles.Manager &&
                  <div className="w-full max-w-full px-3 mb-6 md:w-6/12 md:flex-none xl:mb-0 xl:w-3/12 cursor-pointer">
